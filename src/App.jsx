//App.jsx for the SNHU Map - Places You've Been application
...
async function reverseGeocode(lat, lng) {
   ...
   * Throws on non-OK HTTP responses. Callers should tolerate failures/rate limits.
   */
...
export default function App() {
  ...
  return (
    <div className="app">
      <header>
        <div className="brand">SNHU Map • Places You've Been</div>
        <div className="controls">
          <button onClick={() => setShowList((v) => !v)}>{showList ? "Hide List" : "Show List"}</button>
          <button onClick={() => setMode("done")} disabled={mode === "done" || !hasData}>Done</button>
            ...
            <input
              ...
              onChange={async (e) => {
                try {
                  ...
                  setAlert({
                    kind: "error",
                    msg: err instanceof Error ? err.message : "Failed to load file.",
                  });
                } finally {
            ...
            {locations.map((l) => (
              <div key={l.id} className="item">
                ...
                <div className="item-actions">
                  ...
                  <button onClick={() => setFocus({ lat: l.lat, lng: l.lng })}>Center</button>
                </div>
              </div>
            ))}
              ...
              <Popup>
                <div style={{ display: "grid", gap: 4, maxWidth: 260 }}>
                  ...
                  {l.years && <div><em>{l.years}</em></div>}
                    <button onClick={() => { setEditing(l); setFocus({ lat: l.lat, lng: l.lng }); }}>Edit</button>
                    <button onClick={() => { onDelete(l.id); setFocus({ lat: l.lat, lng: l.lng }); }}>Delete</button>
